...
The test group for ML-DSA / sigGen / FIPS204-tr1 is as follows:
...
| sk | When the test group property `"testType": "AFT"` and `"keyFormat": "expanded"`, the secret key that should be used to generate a signature | hex
| seed | When the test group property `"testType": "AFT"` and `"keyFormat": "seed"`, the seed used to generate the secret key that should be used to generate a signature | hex
...
The following is an example JSON object sent from the server to the client for ML-DSA / sigGen / FIPS204-tr1.
