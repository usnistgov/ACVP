...
* LMS / sigGen / * "AFT" - Algorithm Functional Test. This testing mode expects the IUT to generate valid signatures based on the ACVP provided message. The IUT is *REQUIRED* to generate or use an already-generated key. The IUT is *REQUIRED* to provide the public key and signature to the server. The signature is then validated with the ACVP server. Signatures using the same key will be compared to ensure that each private OTS pair is only used once. 
