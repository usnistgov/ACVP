...
The {spec-algorithm} mode capabilities are advertised as JSON objects within a root "algorithm" object.  The OneStepNoCounter differs from the OneStep KDF only so far as the "l" property is specified on a per auxiliary function basis, there is no counter or loop within the KDF implementation itself, and as such is limited to producing DKM at a maximum length of the output length of the auxiliary function. 
