...
| revision | ACVP Test version | string | See <<supported>>
...
| returnedBitsLen | Returned bits length | integer for "1.0" test revision, domain for "SP800-90Ar1" revision | Min: See <<supported_values>>, Max: 4096.
| counterFieldLen | The length of the counter field in bits. This property is only applicable to ctrDRBG / SP800-90Ar1 (algorithm / revision). | integer | Min: 4, Max: 128 for AES; 64 for TDES
|===
