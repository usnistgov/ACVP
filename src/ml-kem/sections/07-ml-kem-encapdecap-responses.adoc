...
The test groups for ML-KEM / encapDecap / FIPS203-tr1 contain public key properties. The groups can be described using the following table.
...
Each test group contains an array of one or more test cases. Each test case is a JSON object that represents a single test vector to be processed by the ACVP client. The following table describes the JSON elements for each ML-KEM / encapDecap / FIPS203-tr1 test vector.
...
The following is an example JSON test vector response object for ML-KEM / encapDecap / FIPS203-tr1.
