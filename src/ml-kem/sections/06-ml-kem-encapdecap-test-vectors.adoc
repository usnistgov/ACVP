...
The test group for ML-KEM / encapDecap / FIPS203-tr1 is as follows:
...
| keyFormat | The private key format used for decapsulation | string
...
| dk | The decapsulation key when the `"function": "decapsulation" or "decapsulationKeyCheck"` and `"keyFormat": "expanded"` | hex
| seed | The decapsulation key as a seed when `"function": "decapsulation"` and `"keyFormat": "seed"` | hex
...
The following is an example JSON object sent from the server to the client for ML-KEM / encapDecap / FIPS203-tr1.
